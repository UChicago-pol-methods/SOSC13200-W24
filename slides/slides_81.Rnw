...
\item If $\alpha = 0.05$, the probability that we construct a random confidence interval such that the estimand $\theta$ is in our confidence interval is greater than or equal to 0.95.
...
\item Find the lower bound as the $\alpha/2$ percentile, and the upper bound as the $1-\alpha/2$ percentile; i.e., so that $(1-\alpha)$\% of estimates are within this range
\end{enumerate}
